...
So -- by defining various inputs for the bot to make up a state, over time it is able to learn which state-action pairs will lead to good results and act those out on the environment. The 'deep' part of the concept comes from the use of neural networks to resolve these state-action pairs. As you can probably imagine, the computational power needed to define a q-value for every single state-action pair would very quickly become overwhelming, thus the need for a multi-layered neural network. [This demo](https://cs.stanford.edu/people/karpathy/convnetjs/demo/rldemo.html), written by the creator of ConvnetJS, is a great representation of this concept.
